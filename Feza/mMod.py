...
def otomaticMode(directory):
    ...
    while True:
        ...
        bgd_mask = cv2.inRange(img_hsv, np.array([0, 0, 0]), np.array([255, 40, 255]))
        ...
        for i, contour in enumerate(final_contours):
            ...
            cv2.putText(img_bgr, f"{1 if kontur_sayaci == 1 else kontur_sayaci}", (cX, cY), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
